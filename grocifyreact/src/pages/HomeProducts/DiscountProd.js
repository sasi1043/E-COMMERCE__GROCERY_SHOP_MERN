...
async function load(){
    try{
        const res =await axios.get(`${API}/api/discount`);
...
useEffect(()=>{
 ...
 <>
    {/* Navbar */}
      ...
      <h3 className='mt-3'>Products With Discount</h3>
      ...
      <div className='d-flex' >
        ...
        {/* Filter section */}
        <div className='mx-2 mt-2 p-2 rounded head-filter' style={{backgroundColor:theme==="light"?"#fff":"#333",color:theme==="light"?"#222":"#fff"}}>
           <form className='mt-1' >
      ...
      >
        ...
        <div >
          ...
          <div className='ms-5'>
            <div>
              ...
              <label for="tropicana" className='form-label mx-2 '>Flours & Grains</label>
            </div>
            ...
            <div>
              <input type='checkbox' for="tropicana" className='form-check-input border-dark'></input>
              <label for="tropicana" className='form-label mx-2 '>Dairy & Spreads</label>
            </div>
            <div>
              <input type='checkbox' for="tropicana" className='form-check-input border-dark'></input>
              <label for="tropicana" className='form-label mx-2 '>Oils & Ghee</label>
            </div>
          </div>
        </div>
        <h5 className='mt-3'>Brands</h5>
          <div className='ms-5'>
            ...
            <div>
              <input type='checkbox' for="tropicana" className='form-check-input border-dark'></input>
              <label for="tropicana" className='form-label mx-2 '>Daawat</label>
            </div>
...
{/* end of model for filter */}
     ...
     <div className='col-md-9 col-md-3 col-12 mt-2 topmar' style={{backgroundColor:theme==="light"?"#fff":"#333",color:theme==="light"?"#222":"#fff"}}>
      <div className='row gy-4 ms-1 me-1'>
        {discount.map((dr)=>(
          <div className='col-6 col-sm-4 col-md-3' key={dr._id}>
          <div className='card border-secondary w-100 h-100' style={{maxWidth:"250px"}}>
            <div className='card-header' style={{backgroundColor:theme==="light"?"#fff":"#333",color:theme==="light"?"#222":"#fff",textAlign:"center"}}>
              <img src={`${API}/${dr.path}`} alt='apple'  className='img-prod' height={150} width={150}></img>
            </div>
            <div className='card-body' style={{backgroundColor:theme==="light"?"#fff":"#333",color:theme==="light"?"#222":"#fff"}}>
              ...
              <div className='d-flex'>
                <p><s>₹{dr.price}</s></p>
                <h6 className='ms-2 mt-1' style={{color:"green"}}>{dr.discount}% off</h6>

              </div>
              <div className='d-flex justify-content-between  align-items-center'>
                <h6  className='mt-1 fw-bold'>₹{dr.price-(dr.price*(dr.discount/100))}/{dr.measure}</h6>
                <button className='addcart' style={{borderRadius:"25px"}} onClick={()=>{addCart(dr);snackClick()}}>Add To Cart</button>
                <Snackbar
        open={snack}
