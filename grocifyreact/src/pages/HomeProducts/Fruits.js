...
function Fruits() {
  ...
  async function load(){
    try{
      const res=await axios.get(`${API}/api/products/fruits`);
      setFruits(res.data);
    }
    catch(e){
      console.error(e);
  ...
  return (
    ...
    <>
      ...
      <h3 className='mt-3'>Fruits - Fresh and Tasty</h3>
      ...
      >
        ...
        <div >
          ...
          <div className='ms-5'>
           <div>
              ...
              <label for="tropicana" className='form-label mx-2 '>Citrus Fruits</label>
            </div>
           <div>
              <input type='checkbox' for="tropicana" className='form-check-input border-dark'></input>
              <label for="tropicana" className='form-label mx-2 '>Berries</label>
            </div>
           <div>
              <input type='checkbox' for="tropicana" className='form-check-input border-dark'></input>
              <label for="tropicana" className='form-label mx-2 '>Tropical Fruits</label>
            </div>
           <div>
              <input type='checkbox' for="tropicana" className='form-check-input border-dark'></input>
              <label for="tropicana" className='form-label mx-2 '>Pomes</label>
            </div>
          </div>
        </div>
        <h5 className='mt-3'>Brands</h5>
           <div className='ms-5'>
            ...
            <div>
              <input type='checkbox' for="tropicana" className='form-check-input border-dark'></input>
              <label for="tropicana" className='form-label mx-2 '>Sunkits</label>
            </div>
          
           
            <div>
              <input type='checkbox' for="tropicana" className='form-check-input border-dark'></input>
              <label for="tropicana" className='form-label mx-2 '>Zespri</label>
            </div>
          
           
            <div>
              <input type='checkbox' for="tropicana" className='form-check-input border-dark'></input>
              <label for="tropicana" className='form-label mx-2 '>Delight Freshy</label>
            </div>
          
           
            <div>
              <input type='checkbox' for="tropicana" className='form-check-input border-dark'></input>
              <label for="tropicana" className='form-label mx-2 '>Chiquita</label>
            </div>
...
{/* end of model for filter */}
     ...
     <div className='col-md-9 col-md-3 col-12 mt-2 topmar' style={{backgroundColor:theme==="light"?"#fff":"#333",color:theme==="light"?"#222":"#fff"}}>
      <div className='row gy-4 ms-1 me-1'>
        {fruit.map((fr)=>(
          <div className='col-6 col-sm-4 col-md-3' key={fr._id}>
          <div className='card border-secondary w-100 h-100' style={{maxWidth:"250px"}}>
            <div className='card-header' style={{backgroundColor:theme==="light"?"#fff":"#333",color:theme==="light"?"#222":"#fff",textAlign:"center"}}>
              <img src={`${API}/${fr.path}`} alt='apple' className='img-prod' height={150} width={150}></img>
            </div>
            <div className='card-body' style={{backgroundColor:theme==="light"?"#fff":"#333",color:theme==="light"?"#222":"#fff"}}>
              <h6>{fr.name}</h6>
              <p>{fr.description}</p>
              <Rating  name="size-small" defaultValue={fr.rating} size="small" />
              <div className='d-flex justify-content-between  align-items-center'>
                <h6  className='mt-1 fw-bold'>₹ {fr.price}/kg</h6>
                <button className='addcart' style={{borderRadius:"25px"}} onClick={()=>{addCart(fr);snackClick()}}>Add To Cart</button>
                <Snackbar
        open={snack}
