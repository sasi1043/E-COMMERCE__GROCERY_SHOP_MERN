...
function Masala() {
  ...
  const {theme} =useTheme();  
   ...
   const[masala,setMasala]=useState([]);
    ...
    async function load(){
        try{
            const res =await axios.get(`${API}/api/products/masala`);
            setMasala(res.data);
        }
        catch(e){
            console.error(e);
  ...
  return (
    ...
    {/* Navbar */}
      ...
      <h3 className='mt-3'>Pure Spices for Every Dish</h3>
      ...
      <div className='d-flex' >
        ...
        <div className='mx-2 mt-2 p-2 rounded head-filter' >
           <form className='mt-1' >
             ...
             <label style={{marginLeft:"5px"}}  for="Popular">Organic</label>
           </form>
        </div>
        <div className='mx-2 mt-2 p-2 rounded head-filter'>
           <form >
      ...
      >
          ...
          <h5 className='mt-3'>Categories</h5>
           <div className='ms-5'>
            <div>
              ...
              <label for="tropicana" className='form-label mx-2 '>Regional Masala</label>
            </div>
                
            <div>
              <input type='checkbox' for="tropicana" className='form-check-input border-dark'></input>
              <label for="tropicana" className='form-label mx-2 '>Ground Spices</label>
            </div>
            <div>
              <input type='checkbox' for="tropicana" className='form-check-input border-dark'></input>
              <label for="tropicana" className='form-label mx-2 '>Blended Masala</label>
            </div>
            <div>
              <input type='checkbox' for="tropicana" className='form-check-input border-dark'></input>
              <label for="tropicana" className='form-label mx-2 '>Purpose Masala</label>
            </div>
          </div>
          </div>
        ...
        <h5 className='mt-3'>Brands</h5>
         <div className='ms-5'>
            <div>
              <input type='checkbox' for="tropicana" className='form-check-input border-dark'></input>
              <label for="tropicana" className='form-label mx-2 '>Aachi</label>
            </div>
           
            <div>
              <input type='checkbox' for="tropicana" className='form-check-input border-dark'></input>
              <label for="tropicana" className='form-label mx-2 '>Sakthi</label>
            </div>
           
            <div>
              <input type='checkbox' for="tropicana" className='form-check-input border-dark'></input>
              <label for="tropicana" className='form-label mx-2 '>Everest</label>
            </div>
          
           
            <div>
              <input type='checkbox' for="tropicana" className='form-check-input border-dark'></input>
              <label for="tropicana" className='form-label mx-2 '>MTR</label>
            </div>
           
            <div>
              <input type='checkbox' for="tropicana" className='form-check-input border-dark'></input>
              <label for="tropicana" className='form-label mx-2 '>Annapoorna</label>
            </div>
...
{/* end of model for filter */}
      ...
      <div className='container ms-1 me-1 mt-2 rounded' style={{height:"550px",width:"300px",position:"fixed",border:"solid lightgray 1px"}}>
        ...
        <div >
          <h5 className='mt-2 ms-3'>Price Range</h5>
          <div className='ms-5'>
            <div>
              <input type='checkbox' for="tropicana" className='form-check-input border-dark'></input>
              <label for="tropicana" className='form-label mx-2 '>₹0-₹50</label>
     ...
     <div className='col-md-9 col-md-3 col-12 mt-2 topmar'  style={{backgroundColor:theme==="light"?"#fff":"#333",color:theme==="light"?"#222":"#fff"}}>
      <div className='row gy-4 ms-1 me-1'>
        {masala.map((ms)=>(
          <div className='col-6 col-sm-4 col-md-3' key={ms._id}>
          <div className='card border-secondary w-100 h-100' style={{maxWidth:"250px"}}>
            <div className='card-header' style={{backgroundColor:theme==="light"?"#fff":"#333",color:theme==="light"?"#222":"#fff",textAlign:"center"}}>
              <img src={`${API}/${ms.path}`}  className='img-prod' alt='Healthcare' height={150} width={150}></img>
            </div>
            <div className='card-body'  style={{backgroundColor:theme==="light"?"#fff":"#333",color:theme==="light"?"#222":"#fff"}}>
              <h6>{ms.name}</h6>
              <p>{ms.description}</p>
              <Rating  name="size-small" defaultValue={ms.rating} size="small" />
              <div className='d-flex justify-content-between  align-items-center'>
                <h6  className='mt-1 fw-bold'>₹ {ms.price}/pckt</h6>
                <button className='addcart' style={{borderRadius:"25px"}} onClick={()=>{addCart(ms);snackClick()}}>Add To Cart</button>
                <Snackbar
        open={snack}
